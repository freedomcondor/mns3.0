-- Customized Driver,
--    robots with target -1 is also considered arrived
--    this works for when there are extra robots in the morphology, the brain will ignore these extra robots when judging arrival
-- Driver -----------------------------------------
------------------------------------------------------
...
function Driver.step(vns, waiting)
	...
	if goalV2:length() > vns.Parameters.driver_arrive_zone and
	   not (vns.allocator ~= nil and vns.allocator.target ~= nil and vns.allocator.target.idN == -1) then
		-- not arrive
