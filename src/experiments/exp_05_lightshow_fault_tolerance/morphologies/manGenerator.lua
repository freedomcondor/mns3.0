...
function create_shoulder(positionV3, orientationQ, shoulder_angle_Z, shoulder_angle_Y, fore_arm_angle_Z, fore_arm_angle_Y, failure)
	local thick = L * 1.7
	...
	local arm = create_arm(L, L, "red", "red", vector3(0,0,L*0.5), shoulder_orientationQ, fore_arm_angle_Z, fore_arm_angle_Y, failure)
	local node = create_complex_beam(1, L, thick, "red", positionV3, orientationQ * quaternion(math.pi, vector3(1,0,0)), {arm})
	return node
end

function create_arm(L, thick, color, color_fore_arm, positionV3, orientationQ, angleZ, angleY, failure)
...
function create_body(option)
	...
	local node = create_sphere20("blue", "failure_2")
	-- prioritize head
	for idS, childR in pairs(node.children) do
		childR.priority = 2
	end

	local left_shoulder = create_shoulder(vector3(-L*0.1, L*0.25, -L*0.4),
	...
	local right_shoulder = create_shoulder(vector3(-L*0.1, -L*0.25, -L*0.4),
	                                       quaternion(-math.pi/2, vector3(0,0,1)),
	...
	right_shoulder.second_brain = true
