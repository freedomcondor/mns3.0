...
function create_beam_node(L, thick, color, positionV3, orientationQ, last, failure)
local dis = L * 0.5
...
function create_beam_chain(n, L, thick, color, start_position, start_orientation, failure)
	local new_node = create_beam_node(L, thick, color, start_position or vector3(L, 0, 0), start_orientation or quaternion(), n == 1, failure)
	local tail = new_node
	if n ~= 1 then
		local sub_nodes
		sub_nodes, tail = create_beam_chain(n-1, L, thick, color, nil, nil, failure)
		table.insert(new_node.children, sub_nodes)
	end
	return new_node, tail
end

function create_beam(n, L, thick, color, positionV3, orientationQ, children, failure)
	local head_node = {
		robotTypeS = "drone",
		positionV3 = positionV3 or vector3(),
		orientationQ = orientationQ or quaternion(),
	...
	local chain, tail = create_beam_chain(n, L, thick, color, vector3(L, -L*0.5, 0), nil, failure)
	local tail_node = {
		robotTypeS = "drone",
