...
function render_failure(tree, failure)
	tree.failure = failure
	if tree.children ~= nil then
		for id, branch in ipairs(tree.children) do
			render_failure(branch, failure)
		end
	end
end

function create_complex_beam(n, L, thick, color, positionV3, orientationQ, children, failure)
	local half_L = L * 0.5
	local quater_L = L * 0.25
	local half_thick = thick * 0.5
	...
	render_failure(head_node, failure)
