...
drone_locations = generate_random_locations(n_drone,
                                            offset,     yoffset,          # origin location
...
reinforce_offset = [60, -10]

dis = 2
reinforce_drone_locations = []
for i in range(0, 9) :
    for j in range(0, 10) :
        if len(reinforce_drone_locations) < 87 :
...
drone_xml += generate_drones(reinforce_drone_locations, n_drone + 1, 12)                  # from label 1 generate drone xml tags, communication range 4.2
